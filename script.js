...
const revealCallback = (entries, observer) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            // Specifically trigger the dashboard graph drawing animation
            if (entry.target.id === 'hero-dashboard') {
                entry.target.classList.add('animate-graphs');
            }
...
// ==========================================================================
// HERO DASHBOARD: only animate when user scrolls to it (never on page load)
// ==========================================================================
const heroDashboard = document.getElementById('hero-dashboard');
if (heroDashboard) {
    let dashAnimated = false;

    const tryAnimateDashboard = () => {
        if (dashAnimated) return;

        // Guard: user must have scrolled at least 80px — prevents firing on initial load/refresh
        if (window.scrollY < 80) return;

        const rect = heroDashboard.getBoundingClientRect();
        const viewH  = window.innerHeight;

        // Trigger when the dashboard top crosses 85% from the top of the viewport
        if (rect.top < viewH * 0.85 && rect.bottom > 0) {
            dashAnimated = true;
            setTimeout(() => {
                heroDashboard.classList.add('active');
                heroDashboard.classList.add('animate-graphs');
            }, 150);
            window.removeEventListener('scroll', tryAnimateDashboard);
        }
    };

    // Only listen for scroll — never fires if user hasn't scrolled
    window.addEventListener('scroll', tryAnimateDashboard, { passive: true });
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (currentPath !== 'index.html') {
        ...
        // Fallback: If current page is one of the 20 new feature pages
        if (!activeTarget) {
            const activeSubLink = document.querySelector(`.dropdown-sub-panel a[href="${currentPath}"]`);
            if (activeSubLink) {
                const parentPanel = activeSubLink.closest('.dropdown-sub-panel');
                if (parentPanel && parentPanel.id) {
                    activeTarget = parentPanel.id.replace('panel-', '');
                }
            }
        }

        if (activeTarget) {
            // Highlight Solutions parent only if it's a solution/feature page
